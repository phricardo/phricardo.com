...
import { useViewerCookieConsent } from "@/hooks/useViewerCookieConsent";
...
const ArticleDetail = () => {
  ...
  const { hasAcceptedViewerCookies, isViewerCookieReady } =
    useViewerCookieConsent();
  ...
  useEffect(() => {
    if (!hasAcceptedViewerCookies || !isViewerCookieReady) {
      return;
    }
            ...
            <article className="p-0 md:p-0">
              <header className="mb-8 flex flex-col items-center text-center">
                <div className="mb-4 flex flex-wrap items-center justify-center gap-x-4 gap-y-2 text-sm text-github-text">
                  <p>Publicado em {formatDate(data.publishedAt)}</p>
